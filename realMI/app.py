    ...
    if request.method == "POST":
        ...
        for k in request.values.keys():
            if "sell_" in k:
                ...
                agent_id, company_id = agent_id.split("_")
                ...
                if agent_id != "":
                    ...
                    updates["company"] = ObjectId(company_id)
                ...
                home_obj = homes_collection.find_one({"_id": ObjectId(list_home_id)})
                agent_obj = agents_collection.find_one({"_id": ObjectId(list_agent_id)})
    ...
    if request.method == "POST":
        try:
            ...
            if "find_highest_selling_home" in request.values.keys():
                ...
                res = q.find_highest_selling_home(
                    ObjectId(request.values.get("owner_4"))
                )
